...
const Editor = ({ updateCreationCategory, IDProject, TitreSEO, DescriptionSEO, KeywordsSEO }) => {
    ...
    const [previewBackground, setPreviewBackground] = useState("#272822");
    ...
    useEffect(() => {
        const updatePreview = () => {
            const uniqueClass = `codePreview-editor`;
            const scopedCSS = `.${uniqueClass} { ${cssCode} }`;

            setPreview(
                <div className="preview-container" style={{ backgroundColor: previewBackground }}>
                    <label className="containerToggle">
                        <input
                            type="checkbox"
                            checked={previewBackground === "#e8e8e8"}
                            onChange={() => setPreviewBackground(prev => prev === "#272822" ? "#e8e8e8" : "#272822")}
                        />
                    ...
                    <div className={`preview-content ${uniqueClass}`} dangerouslySetInnerHTML={{ __html: HTMLcode }} />
                </div>
            );
        };

        updatePreview();
    }, [HTMLcode, cssCode, previewBackground, IDProject]);


    // eslint-disable-next-line
    const handleBackgroundChange = () => {
        if (previewBackground === "#272822") {
            setPreviewBackground("#e8e8e8");
        } else {
            setPreviewBackground("#272822");
        }
    };
    ...
    return (
        ...
        <div className="containerEditcontainer">
            <div className="containerEdit">
                <div className="sidebar">

                    <div className="Bouton">
                        <div className="A" onClick={() => setJsxTabActive(false)}>
                            <svg
                                width={"16px"}
                                fill="#264de4"
                        ...
                        <div className="B" onClick={() => setJsxTabActive(true)}>
                            <svg
                                width={"20px"}
                                fill="#61DBFB"
                                xmlns="http://www.w3.org/2000/svg"
